<template>
  <Teleport to="body">
    <div
      v-if="modelValue"
      class="fixed inset-0 z-modal flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
      aria-labelledby="upload-modal-title"
      @click.self="handleClose"
    >
      <!-- CORRECCIÓN: en móvil el modal ocupa todo el ancho y se ancla al fondo (sheet pattern) -->
      <div
        class="w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl border bg-background p-6 shadow-2xl
               max-h-[90dvh] flex flex-col"
        @click.stop
      >
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between flex-shrink-0">
          <h2 id="upload-modal-title" class="text-xl font-bold">Subir Archivos</h2>
          <button
        ...
        <div class="overflow-y-auto flex-1 space-y-4 pr-0.5">

          <!-- Drop zone -->
          <!-- CORRECCIÓN: añadido role="button", tabindex y teclado para activar desde teclado -->
          <div
            role="button"
            tabindex="0"
            :aria-disabled="isUploading"
            :aria-label="isDragging ? 'Suelta los archivos aquí' : 'Zona de arrastre. Haz clic o presiona Enter para seleccionar archivos'"
            class="relative cursor-pointer rounded-xl border-2 border-dashed border-primary/30 p-8 sm:p-12 text-center transition-all
                   hover:border-primary/60 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary"
            :class="[
              isDragging ? 'border-primary bg-primary/5' : '',
              isUploading ? 'pointer-events-none opacity-70' : '',
            ]"
            @click="!isUploading && fileInput?.click()"
            @keydown.enter.space.prevent="!isUploading && fileInput?.click()"
            @drop.prevent="onDrop"
            @dragover.prevent
          ...
          >
            ...
            <p class="mb-2 text-base sm:text-lg font-semibold">Arrastra archivos aquí</p>
          ...
          <!-- Progress bar -->
          <!-- CORRECCIÓN: usar <progress> nativo para semántica + soporte lector de pantalla -->
          <div v-if="isUploading || uploadProgress > 0" class="space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium">
                {{ isUploading ? 'Subiendo...' : 'Completado' }}
              </span>
              <span class="text-sm font-semibold text-primary" aria-hidden="true">{{ uploadProgress }}%</span>
            </div>
            <progress
              :value="uploadProgress"
              max="100"
              class="w-full h-2 rounded-full overflow-hidden [&::-webkit-progress-bar]:bg-muted [&::-webkit-progress-value]:bg-primary [&::-moz-progress-bar]:bg-primary"
          ...
          <div v-if="uploadedFiles.length > 0" class="space-y-2">
            ...
            <!-- CORRECCIÓN: max-h en la lista interna, no en el contenedor padre — el modal ya maneja el scroll -->
            <ul class="space-y-2" aria-label="Archivos seleccionados">
              ...
              >
                ...
                <button
                  type="button"
                  :disabled="isUploading"
                  class="flex-shrink-0 rounded p-1.5 text-destructive hover:bg-destructive/10
        ...
        <!-- Footer fijo -->
        <div class="mt-6 flex gap-3 flex-shrink-0 pt-2">
          <button
            type="button"
            :disabled="isUploading"
          ...
          <button
            ...
            type="button"
            :disabled="isUploading"
            class="h-11 flex-1 rounded-lg bg-primary font-medium text-primary-foreground
                   transition-all hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50
...
// CORRECCIÓN: previewUrls como Map normal — no necesita ser ref reactivo de Map completo.
// Se usa un objeto ref simple para forzar reactividad solo al asignar.
...
function createPreview(file: File) {
  ...
  previewUrls.value = new Map(previewUrls.value).set(key, url)
}
...
function validateAndAdd(files: File[]) {
  ...
  for (const file of files) {
    ...
    if (file.size > MAX_SIZE_BYTES) {
      // CORRECCIÓN: incluir nombre + tamaño en el mensaje de error (igual que antes)
  ...
  if (oversized.length) sizeErrors.value = oversized
...
function onDragLeave(e: DragEvent) {
  // CORRECCIÓN: verificar que el cursor salió realmente del área (no de un hijo)
...
async function uploadSequentialFallback(): Promise<BatchUploadResult> {
  let successCount = 0
  ...
  return { successCount, failedCount: total - successCount, uploadedIds: [] }
}
