    ...
    <div class="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
      ...
      <div class="absolute top-0 left-0 right-0 h-16 bg-black/50 backdrop-blur-sm flex items-center justify-between px-6 z-10">
        ...
        <div class="flex items-center gap-2">
          ...
          <button @click="downloadFile" class="p-2 hover:bg-white/10 rounded-lg transition-colors text-white" title="Descargar">
      ...
      <div class="w-full h-full flex items-center justify-center pt-16" @click.self="emit('close')">
        ...
        <div v-else-if="isImage && !hasError" class="max-w-7xl max-h-full p-4">
          <img :src="filePreviewUrl" :alt="document.name" class="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl" @error="handleImageError" />
        </div>

        <!-- VISOR DE PDF -->
        <div v-else-if="isPDF && !hasError" class="w-full h-full max-w-7xl mx-auto p-4">
          <iframe :src="filePreviewUrl" class="w-full h-[85vh] rounded-lg shadow-2xl bg-white" @error="handlePDFError" />
        </div>
        ...
        <div v-else-if="!loadingPreview && !hasError" class="text-center max-w-md">
          ...
          <button @click="downloadFile" class="px-6 py-3 bg-white text-gray-900 rounded-lg font-semibold hover:bg-gray-100 transition-all inline-flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
            Descargar Archivo
          </button>
        </div>
        ...
        <div v-if="hasError" class="text-center max-w-md">
          ...
          <button @click="downloadFile" class="px-6 py-3 bg-white text-gray-900 rounded-lg font-semibold hover:bg-gray-100 transition-all">Descargar en su lugar</button>
        </div>
      </div>
    </div>
  </Teleport>
...
async function downloadFile() {
  const url = await downloadDocument(props.document.id)
  if (!url) return
...
function handleImageError() { hasError.value = true }
function handlePDFError() { hasError.value = true }
...
async function loadPreview() {
  ...
  if (isText.value) {
    ...
    loadingPreview.value = false  // ← agrega esta línea
    return
  }
  ...
  if (isImage.value || isPDF.value) {
    ...
    try {
      const url = await previewDocument(props.document.id)
    ...
    // Tipo no soportado (Word, Excel, etc.)
    loadingPreview.value = false  // ← agrega este bloque
  }
}
