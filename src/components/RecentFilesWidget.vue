  ...
  <div class="p-5 rounded-xl border bg-card">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      <div
        ...
        class="flex flex-col items-center gap-2 p-3 rounded-lg border hover:bg-accent/50 hover:border-primary/30 hover:shadow-sm transition-all cursor-pointer group"
        :title="file.name"
      >
        <!-- Miniatura -->
        <div
          class="w-12 h-12 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 bg-muted group-hover:scale-105 transition-transform"
        >
          <img
          ...
          />
          <span v-else-if="file.type.startsWith('image/')" class="text-2xl"
            >🖼️</span
          >
          <img
        ...
        <!-- Nombre -->
        <p
          class="text-xs font-medium text-center truncate w-full leading-snug group-hover:text-primary transition-colors"
        >
          {{ file.name }}
        </p>
