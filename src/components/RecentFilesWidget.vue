  ...
  <div class="p-5 rounded-xl border bg-card">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      <div
        v-for="file in files"
        :key="file.id"
        class="flex flex-col items-center gap-2 p-3 rounded-lg border hover:bg-accent/50 hover:border-primary/30 transition-all cursor-default"
        :title="file.name"
      >
        <!-- Miniatura real para imágenes, ícono para el resto -->
        <div class="w-12 h-12 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 bg-muted">
          <!-- Imagen: miniatura S3 presignada -->
          <img
          ...
          />
          <!-- Imagen sin miniatura aún: placeholder -->
          <span v-else-if="file.type.startsWith('image/')" class="text-2xl">🖼️</span>
          <!-- Otros tipos: ícono de icons8 -->
          <img
...
// Mismos íconos que Documents.vue
...
// Emoji de respaldo para tipos sin ícono de icons8
