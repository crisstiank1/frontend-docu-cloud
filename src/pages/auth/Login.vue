          ...
          <form class="grid gap-5" @submit.prevent="submit">
            ...
            <div class="grid gap-2">
              <div class="flex items-center justify-between">
                ...
                <router-link to="/auth/reset" class="text-xs text-primary hover:underline">¿Olvidaste tu contraseña?</router-link>
              </div>
              <div class="relative group">
                <div class="absolute left-3 top-1/2 -translate-y-1/2 text-primary/60 group-focus-within:text-primary transition-colors">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
import { useAuditLog } from '../../composables/useAuditLog' // ✅ NUEVO
...
const { login, loginWithGoogle, user } = useAuth() // ✅ AGREGADO: user
const { addLog } = useAuditLog() // ✅ NUEVO
...
async function handleGoogleCallback(response: any) {
  ...
  if (!res.ok) {
    ...
    // ✅ REGISTRAR LOGIN CON GOOGLE EXITOSO
    if (user.value) {
      addLog({
        action: 'login',
        userId: user.value.id,
