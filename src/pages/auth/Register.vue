    ...
    <div class="w-full max-w-md">
      <div class="bg-gradient-to-br from-primary/5 to-accent/5 rounded-2xl border border-primary/10 shadow-lg overflow-hidden">
        <div class="p-8 bg-gradient-to-r from-primary/10 via-transparent to-accent/10 border-b border-primary/10">
          <h1 class="text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Crear cuenta</h1>
          <p class="text-sm text-muted-foreground mt-2">Regístrate para empezar a usar DocuCloud</p>
        </div>
        <div class="p-8">
          <form class="grid gap-5" @submit.prevent="submit">
            <div class="grid gap-2">
              <label class="text-sm font-semibold text-foreground">Nombre completo</label>
              <div class="relative group">
                ...
                <input v-model="name" required placeholder="Tu nombre" class="flex h-11 w-full rounded-lg border border-input bg-background pl-10 pr-4 py-2 text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
            </div>
            <div class="grid gap-2">
            ...
            <div class="grid gap-2">
              ...
              <div class="relative group">
                ...
                <input :type="showPassword ? 'text' : 'password'" v-model="password" required placeholder="Mínimo 6 caracteres" class="flex h-11 w-full rounded-lg border border-input bg-background pl-10 pr-10 py-2 text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
            ...
            <p class="text-sm text-muted-foreground text-center">¿Ya tienes cuenta? <router-link to="/auth/login" class="text-primary font-semibold hover:underline transition-colors">Inicia sesión</router-link></p>
          </form>
        </div>
      </div>
    </div>
