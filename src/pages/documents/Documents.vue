            ...
            >
              <p class="text-sm text-muted-foreground">
                ...
                <span class="hidden sm:inline"
                  >· {{ totalElements }} archivos en total</span
                >
              </p>
...
const {
  ...
  downloadDocument,
  deleteDocument,
  updateDocument, // AGREGADO: usado en saveDocumentChanges
  searchDocuments,
  createFolder,
  renameFolder,
  deleteFolder,
  moveDocumentTo,
  toggleFavorite, // CAMBIO: sin alias ni wrapper — se usa directo en template
...
// AGREGADO: limpieza del timer de búsqueda para evitar memory leak
onUnmounted(() => {
...
// Mapa estático: evita recrear el objeto en cada llamada a getFileIconUrl
...
const selectedDoc = ref<Document | null>(null); // modal de compartir
const viewingDocument = ref<Document | null>(null); // visor de documento
...
// Documentos activos según carpeta/categoría seleccionada
const documentsInCurrentFolder = computed(() => {
  const active = documents.value.filter((d) => d.status !== "DELETED");
  if (currentFolderId.value === null) return active;
  if (currentFolderId.value === "") return getUnclassifiedDocuments();
  return getDocumentsInFolder(currentFolderId.value);
});

// Aplica filtros de búsqueda local, tipo y categoría sobre los docs visibles
const filteredDocuments = computed(() => {
  let docs = documentsInCurrentFolder.value;

  if (currentCategoryId.value)
    docs = docs.filter(
      (d) => String(d.classification?.category) === currentCategoryId.value,
    );
...
// CAMBIO: computed simplificado — elimina allUserDocuments como intermediario
const displayedDocuments = computed(() =>
  showFavoritesOnly.value
    ? filteredDocuments.value.filter((d) => d.isFavorite)
...
// CAMBIO: lookup map en lugar de cadena de if/else
...
// CAMBIO: try/finally garantiza que isUploading siempre se libere
...
// CAMBIO: usa updateDocument() del composable en lugar de mutar documents.value directamente
...
// CAMBIO: eliminada verificación redundante — deleteDocument maneja ids inválidos internamente
...
// CAMBIO: eliminados los bloques vacíos if (user.value) {}
