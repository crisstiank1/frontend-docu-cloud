    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="h-12 px-6 border-b bg-background/50 flex items-center justify-between flex-shrink-0">
          <nav class="flex items-center gap-2 text-sm">
            ...
            <template v-else-if="showUnclassified">
              ...
              <span class="text-muted-foreground font-medium">Sin clasificar</span>
            </template>
          ...
          <div class="flex items-center gap-4 text-sm text-muted-foreground">
            <span>{{ effectiveTotalElements }} archivo{{ effectiveTotalElements !== 1 ? "s" : "" }}</span>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <!-- Loading skeleton -->
          <div v-if="effectiveLoading" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          ...
          <template v-else>
            ...
            <!-- ===== PAGINACIÓN ===== -->
            <div v-if="effectiveTotalPages > 1" class="flex items-center justify-between pt-6 mt-2 border-t">
              ...
              <div class="flex items-center gap-1">
                ...
                <template v-for="page in effectiveTotalPages" :key="page">
                  <button
                    ...
                    :class="page - 1 === effectivePage ? 'bg-primary text-primary-foreground' : 'hover:bg-accent'"
                  >
                    {{ page }}
                  </button>
                </template>
                <button
                  @click="goToPage(effectivePage + 1)"
                  :disabled="effectivePage >= effectiveTotalPages - 1 || effectiveLoading"
                  class="p-2 rounded-lg hover:bg-accent transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  title="Siguiente"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const displayedDocuments = computed(() => {
  ...
  if (!searchQuery.value && !currentFilter.value.type) return viewDocuments.value;
...
const favoriteCount = computed(
  () => documents.value.filter((d) => d.isFavorite && d.status !== "DELETED").length,
);
...
function getFileIconUrl(type: string): string | null {
  ...
  if (type.includes("word") || type.includes("wordprocessingml")) return FILE_ICON_URLS.word;
  if (type.includes("excel") || type.includes("spreadsheet")) return FILE_ICON_URLS.excel;
  if (type.includes("powerpoint") || type.includes("presentation")) return FILE_ICON_URLS.powerpoint;
  return null;
}
...
function getFileIconByName(name: string): string {
  ...
  const map: Record<string, string> = {
    pdf: "📕", doc: "📘", docx: "📘",
    xls: "📊", xlsx: "📊", ppt: "📊", pptx: "📊",
    txt: "📄", jpg: "🖼️", jpeg: "🖼️", png: "🖼️", gif: "🖼️", svg: "🖼️",
  };
  return map[ext] ?? "📎";
}
...
async function refreshCurrentView() {
  if (activeView.value === "all") {
    ...
    await fetchDocumentsByFolder(currentFolderId.value, viewCurrentPage.value, PAGE_SIZE);
  } else if (activeView.value === "favorites") {
    await fetchFavoriteDocuments(viewCurrentPage.value, PAGE_SIZE);
  } else if (activeView.value === "category" && currentCategoryId.value) {
    await fetchDocumentsByCategory(currentCategoryId.value, viewCurrentPage.value, PAGE_SIZE);
  } else if (activeView.value === "unclassified") {
...
function navigateDocument(direction: "prev" | "next") {
  ...
  const idx = displayedDocuments.value.findIndex((d) => d.id === viewingDocument.value!.id);
  if (direction === "prev" && idx > 0) viewingDocument.value = displayedDocuments.value[idx - 1];
...
function shareDoc(email: string, permission: string) {
  if (selectedDoc.value) shareDocument(selectedDoc.value.id, email, permission as "view" | "edit");
}
...
function createLink(permission: string) {
  if (selectedDoc.value) return createShareLink(selectedDoc.value.id, permission === "view");
}
