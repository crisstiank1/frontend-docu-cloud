      ...
      <div v-if="user">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          ...
          <div class="border rounded-xl bg-card overflow-hidden">
            <div
              class="px-6 py-4 border-b bg-muted/30 flex items-center justify-between"
            >
            ...
            <div class="p-6">
              ...
              <form v-else @submit.prevent="saveInfo" class="space-y-4">
                <div>
                  <label
                    class="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1.5 block"
                    >Nombre</label
                  >
                  <input
                ...
                <div>
                  ...
                  <input
                    ...
                    class="w-full h-10 px-3 border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm"
                  />
                  <p v-if="infoError" class="text-xs text-destructive mt-1.5">
                    {{ infoError }}
                  </p>
                </div>
              ...
              <div class="mt-6 pt-6 border-t">
                ...
                <div v-if="!editingPassword">
                  <p class="text-sm text-muted-foreground tracking-widest">
                    ••••••••
                  </p>
                </div>
...
const strengthTextColor = computed(
  () =>
    ["text-red-500", "text-orange-500", "text-yellow-600", "text-green-600"][
      passwordStrength.value - 1
    ] ?? "text-red-500",
);
const strengthLabel = computed(
  () =>
    ["Muy débil", "Débil", "Aceptable", "Segura"][passwordStrength.value - 1] ??
    "Muy débil",
);
...
async function savePassword() {
  ...
  try {
    ...
    setTimeout(async () => {
      await logout(); // ✅ 1. Primero limpiar sesión completa
      await router.replace("/auth/login"); // ✅ 2. Luego navegar — guard ya ve user=null
    }, 2000)
