    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatsCard
          ...
          :subtitle="
            sharedWithMeCount === 0
              ? 'Nadie ha compartido contigo aún'
              : undefined
          "
        >
          <template #icon>
        ...
        <StatsCard
          ...
          :subtitle="
            favoritesCount === 0
              ? 'Marca archivos para acceso rápido'
              : undefined
          "
        >
          <template #icon>
        ...
        <StatsCard
          ...
          unit="MB"
          :trend="
            storagePercent > 80 ? 'down' : storagePercent > 50 ? 'stable' : 'up'
          "
          :accentColor="
            storagePercent > 80
              ? '#ef4444'
...
const sparklineBars = computed(() => {
  ...
  const counts = days.map(
    ({ date }) =>
      logs.value.filter(
        (log) =>
          log.createdAt.startsWith(date) && log.action !== "HTTP_REQUEST",
      ).length,
  );
  ...
  return days.map(({ label }, i) => ({
    ...
    count: counts[i],
    heightPct:
      counts[i] === 0 ? 8 : Math.max(8, Math.round((counts[i] / max) * 100)),
  }));
});
...
const ACTION_MAP: Record<string, string> = {
  // ── Auth ──────────────────────────────────────────────────────────────────
  AUTH_LOGIN_GOOGLE: "login", // ✅ login OAuth2 Google
  AUTH_LOGIN_SUCCESS: "login", // ✅ login normal
  AUTH_LOGIN_FAILURE: "login", // login fallido
  AUTH_LOGIN_BLOCKED: "login", // bloqueado por brute force
  AUTH_LOGOUT: "logout", // ✅ logout
  AUTH_REGISTER: "upload", // registro nuevo usuario
...
async function loadRecentThumbnails(docs: any[]): Promise<void> {
  await Promise.allSettled(
    docs
      .filter(
        (doc) =>
          doc.type.startsWith("image/") && !doc.thumbnailUrl && doc.backendId,
      )
      .map(async (doc) => {
        try {
          const { data } = await api.get(
            `/api/documents/${doc.backendId}/preview`,
          );
...
onMounted(async () => {
  ...
  if (user.value?.roles?.includes("ADMIN")) {
    await Promise.all([fetchUsers(), fetchLogs({}, 0, 20)]);
