...
const { user } = useAuth()
const { categories, folders, loading, totalElements, getSharedWithMe, fetchDocuments } = useDocuments()
const { logs, totalElements: totalLogs, fetchLogs} = useAudit()
...
// ===== FETCH HELPERS (idénticos a tu versión) =====
...
const storagePercent = computed(() => Math.min(Math.round((storageMB.value / 1024) * 100), 100))
</script>
