<template>
  <section class="p-4 sm:p-6 space-y-6">
    ...
    <template v-else>
      ...
      <!-- CORRECCIÓN: flex-wrap para que el botón no se corte en pantallas muy estrechas -->
      <div
        v-if="unclassifiedCount > 0"
        class="flex flex-wrap items-center gap-3 p-4 rounded-xl border border-amber-500/30 bg-amber-500/10"
      >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        >
          <template #icon>
            <svg class="w-4 h-4" fill="none" :stroke="storagePercent > 80 ? '#ef4444' : storagePercent > 50 ? '#f59e0b' : '#10b981'" viewBox="0 0 24 24">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <DashboardCard title="Mis Archivos" description="Ver y gestionar tus archivos" :count="totalElements" countLabel="archivos" to="/files" icon-color="#6366f1" icon-bg-color="#6366f130">
          <template #icon>
        ...
        <DashboardCard title="Clasificación" description="Organiza tus categorías" :count="categories.length" countLabel="categorías" to="/classification" icon-color="#10b981" icon-bg-color="#10b98130">
          <template #icon>
        ...
        <DashboardCard title="Compartidos" description="Archivos compartidos contigo" :count="sharedWithMeCount" countLabel="archivos" to="/shared" icon-color="#ec4899" icon-bg-color="#ec489930">
          <template #icon>
        ...
        <DashboardCard v-if="user?.roles?.includes('ADMIN')" title="Usuarios" description="Gestiona los usuarios del sistema" :count="totalUsers" countLabel="usuarios" to="/users" icon-color="#f59e0b" icon-bg-color="#f59e0b30">
          <template #icon>
        ...
        <DashboardCard v-if="user?.roles?.includes('ADMIN')" title="Historial" description="Auditoría y registro de acciones" :count="totalLogs" countLabel="registros" to="/history" icon-color="#8b5cf6" icon-bg-color="#8b5cf630">
          <template #icon>
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div class="p-5 rounded-xl border bg-card">
            ...
            <div class="flex items-end gap-1 sm:gap-1.5 h-14">
              <div
                v-for="(bar, i) in sparklineBars"
                :key="i"
                class="flex-1 rounded-t-sm transition-all duration-300 hover:opacity-80 min-w-0"
                :style="{ height: `${bar.heightPct}%`, backgroundColor: 'hsl(var(--primary) / 0.7)' }"
                :title="`${bar.label}: ${bar.count} acciones`"
              />
            </div>
            <!-- CORRECCIÓN: overflow-hidden para que las etiquetas no desborden -->
          ...
          <!-- Tips -->
          <div class="p-4 sm:p-5 rounded-xl border bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20">
            <h3 class="font-semibold mb-3 flex items-center gap-2 text-sm">
              <svg class="w-4 h-4 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="p-4 sm:p-5 rounded-xl border bg-card">
            ...
            <div class="space-y-1">
              <router-link to="/files" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-primary/10 text-sm font-medium transition-colors">
                <span class="text-primary">+</span> Subir Archivo
              </router-link>
              <router-link to="/classification" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-primary/10 text-sm font-medium transition-colors">
                <span class="text-primary">+</span> Nueva Categoría
              </router-link>
              <router-link v-if="user?.roles?.includes('ADMIN')" to="/users" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-primary/10 text-sm font-medium transition-colors">
...
const sparklineBars = computed(() => {
  ...
  const counts = days.map(({ date }) =>
    logs.value.filter(log => log.createdAt.startsWith(date) && log.action !== 'HTTP_REQUEST').length
  )
...
const ACTION_MAP: Record<string, string> = {
  LOGIN: 'login', AUTH_LOGIN_GOOGLE: 'login', LOGIN_FAILED: 'login_failed',
  LOGIN_BLOCKED: 'login_blocked', LOGOUT: 'logout', REGISTER: 'register',
  FORGOT_PASSWORD: 'password_change', RESET_PASSWORD: 'password_change',
  UPLOAD_DOCUMENT: 'upload', DOWNLOAD_DOCUMENT: 'download', DELETE_DOCUMENT: 'delete',
  UPDATE_DOCUMENT: 'profile_update', SHARE_DOCUMENT: 'share', REVOKE_SHARE: 'share_revoke',
  UPDATE_SHARE_PERMISSION: 'share_update', CREATE_FOLDER: 'folder_create',
  DELETE_FOLDER: 'folder_delete', RENAME_FOLDER: 'folder_rename',
  CREATE_CATEGORY: 'category_create', UPDATE_CATEGORY: 'category_update',
  DELETE_CATEGORY: 'category_delete', CREATE_TAG: 'tag_add', DELETE_TAG: 'tag_remove',
  UPDATE_USER: 'profile_update', DELETE_USER: 'delete', CHANGE_USER_ROLE: 'role_change',
...
function navigateRecentDocument(direction: 'prev' | 'next') {
  ...
  if (direction === 'prev' && idx > 0) viewingDocument.value = recentFiles.value[idx - 1]
  if (direction === 'next' && idx < recentFiles.value.length - 1) viewingDocument.value = recentFiles.value[idx + 1]
}
...
async function downloadRecentDoc(doc: any) {
  ...
  } catch { console.error('Error al descargar el archivo') }
}
...
onMounted(async () => {
  ...
  if (user.value?.roles?.includes('ADMIN')) {
    ...
    if (isMounted) users.value.forEach((u: any) => {
